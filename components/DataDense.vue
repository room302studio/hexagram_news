<template>
  <div class="hover:bg-zinc-900/10 transition-colors group py-1">
    <div class="flex items-start gap-3 font-mono text-xs">
      ...
      <div
        class="flex-shrink-0 w-[60px] text-zinc-600 text-[9px] uppercase flex gap-1"
      >
        {{ scrap.type || 'unk' }}
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-start gap-2">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex gap-2 text-zinc-500 text-[9px] mt-0.5">
              ...
              <span
                v-if="provider && provider !== getDomain(scrap.url)"
                class="opacity-60"
                >via {{ provider }}</span
              >
              <span v-if="scrap.location" class="opacity-60"
                >📍{{ scrap.location }}</span
              >
            </div>
          </div>
          ...
          <div class="flex-shrink-0 text-[9px] text-zinc-500 flex gap-1">
            <span v-if="scrap.tags?.length" class="opacity-60"
              >{{ scrap.tags.length }}t</span
            >
            <span v-if="scrap.relationships?.length" class="opacity-60"
              >{{ scrap.relationships.length }}r</span
            >
            <span v-if="hasMedia" class="opacity-60">📷</span>
            <span v-if="imageCount > 1" class="opacity-60"
              >{{ imageCount }}img</span
            >
            <span v-if="contentLength" class="opacity-60"
              >{{ contentLength }}ch</span
            >
          </div>
        </div>
        ...
        <!-- Tags inline -->
        <div
          v-if="scrap.tags?.length"
          class="mt-0.5 text-[8px] text-zinc-400 opacity-60"
        >
          <NuxtLink
          ...
          <span v-if="scrap.tags.length > 12"
            >+{{ scrap.tags.length - 12 }}</span
          >
        </div>

        <!-- Summary/Description if compact -->
        <div
          v-if="description && description.length < 100"
          class="mt-0.5 text-zinc-400 text-[9px] leading-relaxed opacity-60"
        >
          {{ description }}
        </div>
      </div>
...
const mostRelevantDate = computed(() => {
  return new Date(
    props.scrap.published_at || props.scrap.updated_at || props.scrap.created_at
  )
})
...
const isToRead = computed(() => {
  return (
    props.scrap.metadata?.toread === true ||
...
const hasEmbedding = computed(() => {
  return !!(
    props.scrap.embedding ||
    props.scrap.embedding_nomic ||
    props.scrap.image_embedding
  )
})

const hasProcessing = computed(() => {
  return !!(
    props.scrap.processing_instance_id || props.scrap.processing_started_at
  )
})

const provider = computed(() => {
  return (
    props.scrap.metadata?.source_data?.provider ||
