...
async function analyzeScraps() {
  ...
  if (scraps.length > 0) {
    ...
    scraps.forEach((scrap, i) => {
      ...
      if (scrap.metadata?.images?.length > 0 || scrap.metadata?.image)
        contentStats.hasMetadataImages++
      if (scrap.tags?.length > 0) contentStats.hasTags++
      if (scrap.source) contentStats.hasSource++
    })

    contentStats.avgContentLength = Math.round(
      totalContentLength / contentStats.hasContent
    )
    contentStats.avgSummaryLength = Math.round(
      totalSummaryLength / contentStats.hasSummary
    )

    console.log(
      'Content field populated:',
      `${contentStats.hasContent}/${scraps.length}`
    )
    console.log(
      'Summary field populated:',
      `${contentStats.hasSummary}/${scraps.length}`
    )
    console.log(
      'Title field populated:',
      `${contentStats.hasTitle}/${scraps.length}`
    )
    console.log(
      'URL field populated:',
      `${contentStats.hasUrl}/${scraps.length}`
    )
    console.log(
      'Screenshot URL populated:',
      `${contentStats.hasScreenshot}/${scraps.length}`
    )
    console.log(
      'Metadata images available:',
      `${contentStats.hasMetadataImages}/${scraps.length}`
    )
    console.log('Tags populated:', `${contentStats.hasTags}/${scraps.length}`)
    console.log(
      'Source populated:',
      `${contentStats.hasSource}/${scraps.length}`
    )
    console.log(
    ...
    const richScrap = scraps.find(
      (s) => s.content && s.summary && s.tags?.length > 0
    )
    if (richScrap) {
      ...
      console.log(
        'Content preview:',
        richScrap.content?.substring(0, 200) + '...'
      )
      console.log(
        'Summary preview:',
        richScrap.summary?.substring(0, 200) + '...'
      )
    ...
    if (linkScrap) {
      ...
      if (linkScrap.metadata) {
        console.log(
          'Metadata keys:',
          Object.keys(linkScrap.metadata).join(', ')
        )
      }
    }
