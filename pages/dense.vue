<template>
  <div class="min-h-screen bg-black text-zinc-100">
        ...
        <div class="flex items-baseline justify-between font-mono text-xs">
          <div class="flex items-baseline gap-4">
            ...
            <span class="text-zinc-500"
              >{{ Object.keys(groupedScraps).length }} groups</span
            >
            <span v-if="isLoading" class="text-zinc-400 animate-pulse"
              >syncing...</span
            >
          </div>
          ...
          <div class="overflow-hidden w-[400px]">
            <div
              class="flex gap-1 text-[8px] text-zinc-500 animate-marquee whitespace-nowrap"
            >
              <span class="text-amber-600">r</span>=toread
              <span class="text-blue-600">s</span>=shared
              <span class="text-green-600">e</span>=embedding
              <span class="text-red-600">p</span>=processing 📷=media
              📍=location #=tags t=tagcount ch=chars img=images pb=pinboard
              gh=github ar=arena md=mastodon tw=twitter yt=youtube
            </div>
          </div>
        </div>
      </div>
    ...
    <main class="container mx-auto px-3 py-4">
      <!-- Loading States -->
      <div
        v-if="isLoading && !scraps.length"
        class="font-mono text-center py-12 text-xs opacity-60"
      >
        Loading scraps...
      </div>
      ...
      <div v-else class="space-y-8">
        <section
          v-for="(group, date) in groupedScraps"
          :key="date"
          class="mb-6"
        >
          <h3 class="font-mono text-xs text-zinc-500 mb-2 opacity-60">
            {{ formatGroupDate(date) }} ({{ group.length }})
          </h3>

          <div class="space-y-0">
            <DataDense
              v-for="scrap in group"
              :key="scrap.id"
              ref="scrapElements"
              :scrap="scrap"
            />
        ...
        <div
          v-if="isLoading"
          class="text-center font-mono text-xs opacity-60 py-4"
        >
          Loading more...
        </div>
      </div>
...
const getMostRelevantDate = (scrap) => {
  return new Date(scrap.published_at || scrap.updated_at || scrap.created_at)
}
...
useHead({
  ...
  meta: [
    {
      name: 'description',
      content: 'A collection of interesting things found around the web'
    }
  ]
})
</script>
